...
if keepBriteBandwith:
    for edge in edges:
        o.write(
            "\t$ns duplex-link $n(" + str(nodes.index(edge[0])) + ") $n(" + str(nodes.index(edge[1])) + ") " + edge[3] + "Mbps " +
            edge[2] + "ms DropTail\n")
else:
    for edge in edges:
        o.write("\t$ns duplex-link $n(" + str(nodes.index(edge[0])) + ") $n(" + str(nodes.index(edge[1])) + ") $linkBW " + edge[
            2] + "ms DropTail\n")
